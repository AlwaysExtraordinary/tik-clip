...
import { getDataJsonFromHandle } from '@/services/fileSystem';
...
export const CoverflowPage: React.FC<CoverflowPageProps> = ({ isVisible = true }) => {
  ...
  // 3. 加载并过滤包含封面的视频数据（若文件夹无封面则不生成 3D 封面）
  const loadCoverflowMovies = useCallback(async () => {
    ...
    try {
      ...
      // 构建 Coverflow 展示对象，并实时补充可能缺失的 data.json 演员/描述元数据
      const loadedMovies: CoverflowMovie[] = await Promise.all(
        coverVideos.map(async (video: Video) => {
          let title = video.name || video.folderName;
          let category = video.category;
          let actors = video.actors;
          let description = video.description;

          if ((!actors || !description) && activeDirectory) {
            try {
              const dataJson = await getDataJsonFromHandle(activeDirectory, video.folderName);
              if (dataJson) {
                if (typeof dataJson.name === 'string' && dataJson.name.trim()) {
                  title = dataJson.name.trim();
                }
                if (typeof dataJson.category === 'string' && dataJson.category.trim()) {
                  category = dataJson.category.trim();
                }
                if (!actors) {
                  if (Array.isArray(dataJson.actors)) {
                    actors = dataJson.actors.filter(
                      (a): a is string => typeof a === 'string' && a.trim() !== ''
                    );
                  } else if (typeof dataJson.actors === 'string' && dataJson.actors.trim()) {
                    actors = dataJson.actors.trim();
                  }
                }
                if (!description && typeof dataJson.description === 'string') {
                  description = dataJson.description.trim();
                }
              }
            } catch {
              // 忽略非致命读取错误
